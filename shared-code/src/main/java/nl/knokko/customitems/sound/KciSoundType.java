...
public class KciSoundType extends ModelValues {

    public static KciSoundType load(BitInput input, ItemSet itemSet) throws UnknownEncodingException {
        byte encoding = input.readByte();
        if (encoding != 1) throw new UnknownEncodingException("SoundType", encoding);

        KciSoundType result = new KciSoundType(false);
        ...
        result.soundCategory = VSoundCategory.valueOf(input.readString());
        return result;
    }
    ...
    private VSoundCategory soundCategory;

    public KciSoundType(boolean mutable) {
        super(mutable);
        this.id = UUID.randomUUID();
        this.name = "";
        this.oggData = null;
        this.soundCategory = VSoundCategory.MASTER;
    }

    public KciSoundType(KciSoundType toCopy, boolean mutable) {
        super(mutable);
    ...
    @Override
    public KciSoundType copy(boolean mutable) {
        return new KciSoundType(this, mutable);
    }

    @Override
    public boolean equals(Object other) {
        if (other instanceof KciSoundType) {
            KciSoundType otherSound = (KciSoundType) other;
    ...
    public VSoundCategory getSoundCategory() {
    ...
    public void setSoundCategory(VSoundCategory soundCategory) {
        assertMutable();
