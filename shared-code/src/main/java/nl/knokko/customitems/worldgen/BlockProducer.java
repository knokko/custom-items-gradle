...
public class BlockProducer extends ModelValues {

    public static BlockProducer load(BitInput input, ItemSet itemSet) throws UnknownEncodingException {
        byte encoding = input.readByte();
        if (encoding != 1) throw new UnknownEncodingException("BlockProducer", encoding);

        BlockProducer result = new BlockProducer(false);
    ...
    public static BlockProducer createQuick(Entry... entries) {
        BlockProducer result = new BlockProducer(true);
    ...
    public static BlockProducer createQuick(List<Entry> entries) {
        BlockProducer result = new BlockProducer(true);
    ...
    public BlockProducer(boolean mutable) {
        super(mutable);
        this.entries = new ArrayList<>(1);
        this.entries.add(new Entry(false));
    }

    public BlockProducer(BlockProducer toCopy, boolean mutable) {
        super(mutable);
    ...
    @Override
    public BlockProducer copy(boolean mutable) {
        return new BlockProducer(this, mutable);
    }

    @Override
    public boolean equals(Object other) {
        return other instanceof BlockProducer && this.entries.equals(((BlockProducer) other).entries);
    }
    ...
    public static class Entry extends ModelValues {
        ...
        public Entry(boolean mutable) {
            super(mutable);
            this.block = new ProducedBlock(VMaterial.COAL_ORE);
