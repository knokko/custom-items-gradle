...
public class OreGenerator extends ModelValues {

    public static OreGenerator load(BitInput input, ItemSet itemSet) throws UnknownEncodingException {
        byte encoding = input.readByte();
        if (encoding < 1 || encoding > 2) throw new UnknownEncodingException("OreVeinGenerator", encoding);

        OreGenerator result = new OreGenerator(false);
        result.blocksToReplace = ReplaceBlocks.load(input, itemSet);
        result.allowedBiomes = AllowedBiomes.load(input);
        result.oreMaterial = BlockProducer.load(input, itemSet);
    ...
    private ReplaceBlocks blocksToReplace;
    private AllowedBiomes allowedBiomes;
    private List<String> allowedWorlds;

    private BlockProducer oreMaterial;
    ...
    public OreGenerator(boolean mutable) {
        super(mutable);
        this.blocksToReplace = new ReplaceBlocks(false);
        this.allowedBiomes = new AllowedBiomes(false);
        this.allowedWorlds = Collections.emptyList();
        this.oreMaterial = new BlockProducer(false);
        this.minY = 20;
        this.maxY = 40;
    ...
    public OreGenerator(OreGenerator toCopy, boolean mutable) {
        super(mutable);
    ...
    @Override
    public OreGenerator copy(boolean mutable) {
        return new OreGenerator(this, mutable);
    }

    @Override
    public boolean equals(Object other) {
        if (other instanceof OreGenerator) {
            OreGenerator otherVein = (OreGenerator) other;
    ...
    public String toString() {
        Optional<BlockProducer.Entry> maxEntry = oreMaterial.getEntries().stream().max(
    ...
    public ReplaceBlocks getBlocksToReplace() {
    ...
    public BlockProducer getOreMaterial() {
        return oreMaterial;
    }
    ...
    public void setBlocksToReplace(ReplaceBlocks blocksToReplace) {
        assertMutable();
    ...
    public void setOreMaterial(BlockProducer oreMaterial) {
        assertMutable();
