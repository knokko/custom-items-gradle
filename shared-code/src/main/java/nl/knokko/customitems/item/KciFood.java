...
public class KciFood extends KciItem {

    static KciFood load(
            BitInput input, byte encoding, ItemSet itemSet
    ) throws UnknownEncodingException {
        KciFood result = new KciFood(false);
    ...
    private Collection<KciPotionEffect> eatEffects;
    private int eatTime;

    private KciSound eatSound;
    private int soundPeriod;

    private byte maxStacksize;

    public KciFood(boolean mutable) {
        super(mutable, KciItemType.DIAMOND_HOE);

        this.foodValue = 4;
        this.eatEffects = new ArrayList<>();
        this.eatTime = 30;
        this.eatSound = KciSound.createQuick(VSoundType.ENTITY_GENERIC_EAT, 1f, 1f).copy(false);
        this.soundPeriod = 10;
        this.maxStacksize = 64;
    }

    public KciFood(KciFood toCopy, boolean mutable) {
    ...
    protected void loadFoodPropertiesNew(BitInput input, ItemSet itemSet) throws UnknownEncodingException {
        ...
        if (encoding == 1) {
            VSoundType eatSoundType = VSoundType.valueOf(input.readString());
            ...
            this.eatSound = KciSound.createQuick(eatSoundType, eatSoundVolume, eatSoundPitch).copy(false);
        } else {
            this.foodValue = input.readInt();
            this.eatSound = KciSound.load(input, itemSet);
            this.eatTime = input.readInt();
            this.soundPeriod = input.readInt();
            this.maxStacksize = input.readByte();
        }
    ...
    private void loadFoodOnlyProperties10(BitInput input) {
        ...
        for (int counter = 0; counter < numEatEffects; counter++) {
            this.eatEffects.add(KciPotionEffect.load2(input, false));
        }
        this.eatTime = input.readInt();
        this.eatSound = KciSound.createQuick(VSoundType.valueOf(input.readString()), input.readFloat(), input.readFloat()).copy(false);
        this.soundPeriod = input.readInt();
    ...
    protected boolean areFoodPropertiesEqual(KciFood other) {
    ...
    public boolean equals(Object other) {
        return other.getClass() == KciFood.class && areFoodPropertiesEqual((KciFood) other);
    }

    @Override
    public KciFood copy(boolean mutable) {
        return new KciFood(this, mutable);
    }
    ...
    public Collection<KciPotionEffect> getEatEffects() {
    ...
    public void setEatEffects(Collection<KciPotionEffect> newEatEffects) {
        assertMutable();
    ...
    public void setEatSound(KciSound newEatSound) {
        assertMutable();
    ...
    public void validateIndependent() throws ValidationException, ProgrammingValidationException {
        ...
        for (KciPotionEffect eatEffect : eatEffects) {
    ...
    public void validateExportVersion(int version) throws ValidationException, ProgrammingValidationException {
        for (KciPotionEffect effect : eatEffects) {
