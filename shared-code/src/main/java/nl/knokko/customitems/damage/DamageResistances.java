...
public class DamageResistances {
	...
	public static DamageResistances load17(BitInput input) {
		DamageResistances resistances = new DamageResistances();
		for (int index = 0; index < AMOUNT_17; index++) {
			if (input.readBoolean()) {
				resistances.resistanceMap[index] = input.readShort();
			}
		}
	...
	public DamageResistances() {
		this.resistanceMap = new short[AMOUNT_17];
	}
	
	public DamageResistances(short[] resistanceMap) {
		if (resistanceMap.length > AMOUNT_17) {
	...
	public DamageResistances clone() {
		...
		System.arraycopy(resistanceMap, 0, clone.resistanceMap, 0, AMOUNT_17);
		return clone;
	}
