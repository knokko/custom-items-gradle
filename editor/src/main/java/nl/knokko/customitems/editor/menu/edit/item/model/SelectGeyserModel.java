...
public class SelectGeyserModel extends GuiMenu {
    ...
    protected void addComponents() {
        ...
        addComponent(new DynamicTextComponent("Attachable ID:", LABEL), 0.25f, 0.7f, 0.4f, 0.8f);
        TextEditField idField = new TextEditField("", EDIT_BASE, EDIT_ACTIVE);
        addComponent(idField, 0.45f, 0.7f, 0.6f, 0.8f);

        byte[][] files = { null, null, null };
        addComponent(new DynamicTextButton("Select animation file...", BUTTON, HOVER, () -> {
            selectFile(0, files, errorComponent);
        }), 0.25f, 0.55f, 0.5f, 0.65f);
        addComponent(new DynamicTextButton("Select attachable file...", BUTTON, HOVER, () -> {
            selectFile(1, files, errorComponent);
        }), 0.25f, 0.4f, 0.5f, 0.5f);
        addComponent(new DynamicTextButton("Select model file...", BUTTON, HOVER, () -> {
            selectFile(2, files, errorComponent);
        ...
        addComponent(new ConditionalTextButton("Done", SAVE_BASE, SAVE_HOVER, () -> {
            consumeModel.accept(new GeyserCustomModel(idField.getText(), files[0], files[1], files[2]));
            state.getWindow().setMainComponent(returnMenu);
        }, () -> files[0] != null && files[1] != null && files[2] != null && !idField.getText().isEmpty()),
    ...
    private void selectFile(int index, byte[][] files, DynamicTextComponent errorComponent) {
