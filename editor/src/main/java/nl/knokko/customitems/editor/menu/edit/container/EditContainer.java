...
public class EditContainer extends GuiMenu {
	...
	public EditContainer(
			ItemSet itemSet, GuiComponent returnMenu, KciContainer oldValues, ContainerReference toModify
	) {
	...
	protected void addComponents() {
		...
		addComponent(new DynamicTextButton("Change...", BUTTON, HOVER, () -> {
			state.getWindow().setMainComponent(new CreateDisplay(this, itemSet, newSelectionIcon -> {
		...
		addComponent(
				new DynamicTextButton("Change...", BUTTON, HOVER, () -> {
					state.getWindow().setMainComponent(new ChooseContainerHost(this, newHost -> {
		...
		addComponent(new DynamicTextButton("Recipes...", BUTTON, HOVER, () -> {
			state.getWindow().setMainComponent(new ContainerRecipeCollectionEdit(
					itemSet, currentValues, this
			));
		}), 0.05f, 0.15f, 0.2f, 0.2f);
		
		addComponent(
				new SlotsComponent(this, itemSet, currentValues),
				0.36f, 0.1f, 1f, 0.9f
		);
		
		if (toModify != null) {
			addComponent(new DynamicTextButton("Apply", SAVE_BASE, SAVE_HOVER, () -> {
				String error = Validation.toErrorString(() -> itemSet.containers.change(toModify, currentValues));
			...
			addComponent(new DynamicTextButton("Create", SAVE_BASE, SAVE_HOVER, () -> {
				String error = Validation.toErrorString(() -> itemSet.containers.add(currentValues));
				if (error != null) {
