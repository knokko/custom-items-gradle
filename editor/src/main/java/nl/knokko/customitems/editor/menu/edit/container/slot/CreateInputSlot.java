...
public class CreateInputSlot extends GuiMenu {
	...
	public CreateInputSlot(GuiComponent returnMenu, Consumer<ContainerSlotValues> submitSlot,
	...
	protected void addComponents() {
		...
		addComponent(new DynamicTextButton("Done", EditProps.SAVE_BASE, EditProps.SAVE_HOVER, () -> {
			...
			for (ContainerSlotValues existingSlot : existingSlots) {
				if (existingSlot instanceof InputSlotValues) {
					InputSlotValues existingInputSlot = (InputSlotValues) existingSlot;
					if (
			...
			submitSlot.accept(InputSlotValues.createQuick(nameField.getText(), pPlaceholder[0]));
			state.getWindow().setMainComponent(returnMenu);
