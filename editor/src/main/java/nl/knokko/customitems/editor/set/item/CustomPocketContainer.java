...
public class CustomPocketContainer extends CustomItem {
    ...
    private Collection<String> containerNames;
    ...
    public CustomPocketContainer(
            ...
            List<PotionEffect> targetEffects, Collection<EquippedPotionEffect> equippedEffects, String[] commands,
            ReplaceCondition[] conditions, ReplaceCondition.ConditionOperation op,
            ExtraItemNbt extraNbt, float attackRange,
            Collection<String> containerNames, Collection<CustomContainer> containers) {
        super(
    ...
    public Collection<CustomContainer> getContainers() {
        return containers;
    }

    public void setContainers(Collection<CustomContainer> newContainers) {
        this.containers = new ArrayList<>(newContainers);
    }
    ...
    public void export(BitOutput output) {
        ...
        for (CustomContainer container : containers) {
            output.addString(container.getName());
        }
    }

    public void findContainers(ItemSet set) throws IllegalArgumentException {
        this.containers = new ArrayList<>(this.containerNames.size());
        for (CustomContainer container : set.getBackingContainers()) {
        ...
        if (containers.size() != containerNames.size()) {
            throw new IllegalArgumentException("Couldn't find all containers: " + containerNames);
        }
